...
function App() {
  ...
  const derived = useMemo(() => {
    ...
    const groupsForSummary = selectedGroup === 'All Groups' ? allSummaryGroups : [selectedGroup].filter(Boolean);
    ...
    const overallNpsByIntent = intents.map((intent) => {
      ...
      return {
        ...
        [`${selectedPeriod}Stats`]: currentStats,
    ...
    const buildNpsCell = (group, intent) => {
      const currentStats = npsStatsFor(selectedPeriod, group, intent, respFilterFor(selectedPeriod, group));
    ...
    const respondentPeriodComparison = [previousPeriod, selectedPeriod].filter(Boolean).map((period) => {
    ...
    const buildComparisonCounts = (currentRows, previousRows, getter) => {
      ...
      return keys.map((name) => ({
        name,
        [selectedPeriod]: current.find((d) => d.name === name)?.count || 0,
  ...
  }, [data, selectedPeriod, selectedGroup, demoFilters, npsDetailGroup, compareEnabled, sentimentFilter, npsMixIntent]);
                ...
                <div className="chart-height small">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={derived.extracurricularCounts}>
                      ...
                      <Bar dataKey={selectedPeriod} name={selectedPeriod} fill={CURRENT_COLOR} />
                      <Legend />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
